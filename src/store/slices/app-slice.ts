...
import {  SafLibContract, LpReserveContract, Erc20Contract } from "../../abi";
...
export const loadAppDetails = createAsyncThunk(
    ...
    async ({ networkID, provider }: ILoadAppDetails) => {
      ...
      //  const marketPrice = (await getMarketPrice(networkID, provider)) * bnbPrice;
        ...
        const totalSupply = (await safuliberoContract.totalSupply()) / Math.pow(10, 5);
        const circSupply = totalSupply - (await safuliberoContract.balanceOf(addresses.FIREPIT_ADDRESS)) / Math.pow(10,5);
        ...
        const treasurySafLibValue = (await safuliberoContract.balanceOf(addresses.TREASURY_ADDRESS) / Math.pow(10,5)) * marketPrice ;
        ...
        const treasuryValue = treasuryBNBValue + treasurySafLibValue + treasuryBUSDValue;
        const backedLiquidity = "100%";
        const firepitBalance = (await safuliberoContract.balanceOf(addresses.FIREPIT_ADDRESS))/ Math.pow(10,5);
